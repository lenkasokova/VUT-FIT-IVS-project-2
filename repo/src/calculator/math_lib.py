...
def func_sum(a, b):
    ...
    @return (int or float): Return value (int or float) if everything went well, otherwise return ValueError
    """
...
def root(number, sqrt_root=2):
    ...
    root_num = number ** (1 / sqrt_root)
    # this block od codes is because python giving complex numbers for negative numbers
    val = 0
    if number < 0:
        a = root_num.real ** 2
        b = root_num.imag ** 2
        return round(-math.sqrt(a + b), 6)
